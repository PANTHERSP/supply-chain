...
const YourProductsPage = () => {
    ...
    const fetchProducts = async () => {
      ...
      const filteredProducts = products.filter(product => product.history[0].status === 'customer receive product' && product.dealId === selectedDeal?.dealId);
        ...
        <>
            ...
            {/* {selectedProduct && (
                <div ref={modalRef} className="fixed top-0 left-0 w-full h-full bg-black/85 flex justify-center z-50">
                    <div className="bg-gradient-to-r from-indigo-950 via-gray-900 to-gray-800 p-8 rounded-3xl max-w-md w-full text-sky-100 overflow-auto my-5">
                        ...
                        { selectedProduct.history && 
                            <div className="flex flex-col mt-10 font-semibold border-t-2 border-sky-100 text-lg py-4 gap-6">
                                {
                                    selectedProduct.history.map((history, index) => (
                                        <div key={index} className="flex flex-col border-dashed border-b-2 border-amber-200 pb-4">
                                            <p className={`${index === 0 && 'text-amber-500' }`}>Status: {history.status}</p>
                                            <p className={`${index === 0 && 'text-amber-500' }`}>Date: {history.date}</p>
                                            <p className={`text-sm ${index === 0 && 'text-amber-500' }`}>Evidence: {history.evidence}</p>
